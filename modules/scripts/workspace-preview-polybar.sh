...
# Generate workspace preview for Polybar
preview=""
for i in {1..10}; do
            ...
            while IFS= read -r window_id; do
                if [ -n "$window_id" ]; then
                    ...
                    if [ -n "$window_class" ]; then
                        ...
                        app_icons="${app_icons}%{F#a0aec0}$icon_result%{F-}"
                    fi
                fi
            done <<< "$window_ids"
        fi
    ...
    if [ "$ws_num" -eq "$((current_ws + 1))" ]; then
        ...
        if [ -n "$app_icons" ]; then
            preview="${preview}%{A:xdotool key super+$key:}%{F#68d391}%{T2}$ws_num[$app_icons]%{T-}%{F-}%{A} "
        else
            preview="${preview}%{A:xdotool key super+$key:}%{F#68d391}%{T2}$ws_num[]%{T-}%{F-}%{A} "
        fi
    else
        # Other workspaces - normal
        if [ -n "$app_icons" ]; then
            preview="${preview}%{A:xdotool key super+$key:}%{F#a0aec0}$ws_num[$app_icons]%{F-}%{A} "
        else
            preview="${preview}%{A:xdotool key super+$key:}%{F#4a5568}$ws_num[]%{F-}%{A} "
        fi
    fi
done
